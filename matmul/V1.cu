...
/*
    最简单的矩阵乘法，每一个线程计算一个元素 c(i, j).
    缺点：没有利用到 共享内存，全局内存的加载负荷极大，访存受限.
*/

__global__ void mysgemm_v1(int M, int N, int K, float alpha, float *A, float *B, float beta, float *C) {
    
  // A 的形状为(M, K) , B的形状为(K, N)
  int gy = blockIdx.y * blockDim.y + threadIdx.y; //  行号 y
  int gx = blockIdx.x * blockDim.x + threadIdx.x; //  列号 x
